        ...
        while True:
            ...
            for b in curently_tracking:
                ...
                try:
                    ...
                    print("Error in :"+str(b.error)+"    "+datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                    b.tries += 1
                    b.lost = 1
                    await asyncio.sleep(10)
                if  b.mss =="" and b.tries > 8 and b.lost ==1:
                        b.mss = message.channel.send("server ( {0} ) is down, can't connect to rcon...<@&{1}> please fix it up dog".format(b.ip+":"+str(b.game_port), str(server_main)))
                ...
                if b.lost == 0 and b.mss !="":
                        ...
                        await message.channel.send("Fixed server ( {0} )".format(b.ip+":"+str(b.game_port)))
                kkll = 0
                yes = False
                ...
                if b.timer > (60*20/5) and int(b.players_count) > 0 and yes == True:
...
############################################################################################################################################################################################
    ...
    if message.content.startswith('!say') and (message.author.id == main_guy or admin_privelege == True or message.guild.owner == message.author):
        ...
        for i in current_serv:
            m += "["+Classes.numbers(count)+"]"+" {0}".format(i.ip+":"+str(i.game_port)+"  | --> "+b.full_name+"\n")
